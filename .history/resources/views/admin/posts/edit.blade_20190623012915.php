...
<input type="file" name="image" id="image" value="{{$post->image}}" class="col-7 form-control @error('image') is-invalid @enderror" >
  ...
  <div class="form-check">
    <input class="form-check-input" type="checkbox"  id="category_id" name="category_id[]" value="{{$category->id}}"@foreach($post->categories as $Pcategory) @if($Pcategory->category_id==$category->id)
     checked
  @endif
    @endforeach
     > {{$category->title}}
