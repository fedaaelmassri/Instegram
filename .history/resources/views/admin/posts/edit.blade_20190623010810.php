...
<input type="file" name="image" id="image" value="{{$post->image}}" class="col-7 form-control @error('image') is-invalid @enderror" >
  ...
  <div class="form-check">
    ...
    <input class="form-check-input" type="radio" name="status"  value="Published"  @if($post->status=='published') checked
  @endif> Published
  </div>
  <div class="form-check">
    <input class="form-check-input" type="radio"name="status" value="Draft" @if($post->status=='published') checked
  @endif > Draft
