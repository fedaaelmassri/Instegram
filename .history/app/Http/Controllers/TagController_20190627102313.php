...
class TagController extends Controller
{
 ...
 //return  view('admin.tags.index',[
    ...
    // $tags=tag::withoutGlobalScope('published')
     ...
     // ->groupBy('status')
     // ->get()
     $tags=Tag::when($title,function($query,$tag){
 ...
 return  view('admin.tags.index',[
     ...
     public function forceDelete($id){
         ...
         ->with('message', sprintf('tag "%s" Deleted!', $tag->title));
     ...
     public function restore(Request $request,$id){
         ...
         ->with('message', sprintf('tag "%s" Restore!', $tag->title));
     ...
     public function store(Request $request){
      ...
      foreach($category as $cat){
         ...
         return redirect( route('admin.tags') )
         ->with('message', sprintf('tag "%s" created!', $tag->title));
     ...
     //    $category_tag=new category_tag();
         ...
         return redirect(route('admin.tags'))->with(
             'message',sprintf('tag "%s" updated!',$tag->title)
         );
     ...
     public function delete($id){
         ...
         $tag->delete();
         return redirect(route('admin.tags'))->with(
             'message',sprintf('tag "%s" deleted!',$tag->title)
         );
